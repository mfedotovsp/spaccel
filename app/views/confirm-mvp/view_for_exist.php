        ...
        <div class="row row_header_data">
            ...
            <div class="col-md-3" style="padding: 0;">
                ...
                <?php if (!$model->getDeletedAt() ||
                    ...
                    <?= Html::a( 'Далее', ['/business-model/index', 'id' => $model->getId()],[
                        'style' => [
                            'display' => 'flex',
                            'align-items' => 'center',
                ...
                <?php if (!$model->getDeletedAt() && User::isUserExpert(Yii::$app->user->identity['username'])) : ?>
                    ...
                    <?php if (ProjectCommunications::checkOfAccessToCarryingExpertise(Yii::$app->user->getId(), $model->hypothesis->getProjectId())) : ?>
                        ...
                        <?= Html::a('Экспертиза',['/expertise/get-list', 'stage' => StageExpertise::getList()[StageExpertise::CONFIRM_MVP], 'stageId' => $model->getId()], [
...
<div class="confirm-hypothesis-view-mobile">
    ...
    <div class="arrow_links_router_mobile">
        ...
        <div class="arrow_link_router_mobile_right">
            <?php if ($mvp->getExistConfirm() === StatusConfirmHypothesis::COMPLETED && $model->getEnableExpertise() === EnableExpertise::ON) : ?>
                <?php if (!$model->getDeletedAt() ||
                    (BusinessModel::find(false)
                        ->where(['basic_confirm_id' => $model->getId()])
                        ->exists())): ?>
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <?php else : ?>
                ...
                <?= Html::a('Смотреть экспертизу',['/expertise/get-list', 'stage' => StageExpertise::getList()[StageExpertise::CONFIRM_MVP], 'stageId' => $model->getId()], [
                    'class' => 'link-get-list-expertise btn btn-lg btn-default',
                    'style' => [
                        'display' => 'flex',
                        'width' => '100%',
...
<?php else: ?>
    ...
    <div class="container-fluid">
        ...
        <div class="row pt-20 pb-5 pl-5">
            <div class="col-md-12 bolder">Цель проекта</div>
            <div class="col-md-12 mb-5"><?= $project->getPurposeProject() ?></div>
            <div class="col-md-12 bolder">Формулировка минимально жизнеспособного продукта, который проверяем</div>
            <div class="col-md-12 mb-5"><?= $mvp->getDescription() ?></div>
        </div>

        <?php
        $form = ActiveForm::begin([
            'id' => 'update_confirm_mvp',
            'action' => Url::to(['/confirm-mvp/update', 'id' => $model->getId(), 'existDesc' => true]),
...
<?php
$this->registerJsFile('@web/js/confirm_mvp_view_for_exist.js');
