        ...
        <div class="row row_header_data">
            ...
            <div class="col-md-3" style="padding: 0;">
                ...
                (Problems::find(false)
                    ...
                    <?= Html::a( 'Далее', ['/problems/index', 'id' => $model->getId()],[
                        'style' => [
                            'display' => 'flex',
                            'align-items' => 'center',
                ...
                <?php if (!$model->getDeletedAt() && User::isUserExpert(Yii::$app->user->identity['username'])) : ?>
                    ...
                    <?php if (ProjectCommunications::checkOfAccessToCarryingExpertise(Yii::$app->user->getId(), $model->hypothesis->getProjectId())) : ?>
                        ...
                        <?= Html::a('Экспертиза',['/expertise/get-list', 'stage' => StageExpertise::getList()[StageExpertise::CONFIRM_SEGMENT], 'stageId' => $model->getId()], [
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <?php else : ?>
                ...
                <?= Html::a('Смотреть экспертизу',['/expertise/get-list', 'stage' => StageExpertise::getList()[StageExpertise::CONFIRM_SEGMENT], 'stageId' => $model->getId()], [
                    'class' => 'link-get-list-expertise btn btn-lg btn-default',
                    'style' => [
                        'display' => 'flex',
                        'width' => '100%',
...
<?php if (!User::isUserSimple(Yii::$app->user->identity['username']) || $model->getDeletedAt()): ?>
    ...
    <div class="container-fluid">
        ...
        <div class="row pt-20 pb-5 pl-5">
            ...
            <?php foreach ($model->confirmDescription->problemVariants as $i => $problemVariant): ?>

                <div class="col-md-12 mb-10">
                    <div>
                        <span class="bolder"><?= ($i+1) ?>.</span>
                        <?= $problemVariant->getDescription() ?>
                    </div>
                </div>
...
<?php else: ?>
    ...
    <div class="container-fluid">
        ...
        $form = ActiveForm::begin([
            'id' => 'update_confirm_segment',
            'action' => Url::to(['/confirm-segment/update', 'id' => $model->getId(), 'existDesc' => true]),
        ...
        <div class="container-problem-variants">
            <div class="item-variants p-0">
                <?php foreach ($formUpdate->problemVariants as $key => $value): ?>

                    <div class="row row-variant row-variant-form-<?= $key ?> mb-5">

                        <?= $form->field($formUpdate, "problemVariants[".$key."][id]")->hiddenInput()->label(false) ?>
                        <?= $form->field($formUpdate, "problemVariants[".$key."][description]", [
                        ...
                        <?php if ($key > 0): ?>
                            <div class="col-md-12 mt-15">
                                ...
                                <?= Html::button('Удалить проблему', [
                                    ...
                                    'class' => 'remove-problem-variant btn btn-default remove_problem_variant_for_update',
                                    'style' => [
                                        'display' => 'flex',
                                        'align-items' => 'center',
                                        'justify-content' => 'center',
        ...
        <?= Html::button('Добавить проблему', [
            ...
            'class' => 'btn btn-default add_problem_variant_form',
            'style' => [
                'display' => 'flex',
                'align-items' => 'center',
                'color' => '#FFFFFF',
...
<div class="form_problem_variants display-none">
    ...
    <div class="form_problem_variants_inputs">
        ...
        <div class="row mb-15 row-variant row-variant-">

            <?= $form->field(new ProblemVariant(), "[0]description", [
                'template' => '<div class="col-md-12 mb-15 mt-15">{input}</div>'
            ])->textarea([
                'rows' => 2,
                'maxlength' => 2000,
                'placeholder' => '',
...
<?php
$this->registerJsFile('@web/js/confirm_segment_view_for_exist.js');
