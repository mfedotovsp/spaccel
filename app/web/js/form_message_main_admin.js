...
$(body).on('beforeSubmit', '#create-message-main-admin', function (e) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.sender === 'admin') $(conversation_list_menu).find(response.blockConversationAdminMain).html(response.conversationAdminMainForAdminAjax);
            else if (response.sender === 'main_admin') $(conversation_list_menu).find('.containerForAllConversations').html(response.conversationsAdminForAdminMainAjax);
            ...
            $('#create-message-main-admin')[0].reset();
...
$(body).on('input', 'textarea#input_send_message', function () {
    ...
    var url = '/' + module + '/message/save-cache-message-main-admin-form?id=' + conversation_id;
...
$(body).on('click', '.pagination-messages .messages-pagination-list li.next a', function(e){
    ...
    var url = '/' + module + '/message/get-page-message?id=' + conversation_id + '&page=' + next_page + '&final=' + idFirstMessagePreviosPage;
...
setInterval(function(){
    ...
    var url = '/' + module + '/message/check-new-messages-main-admin?id=' + conversation_id + '&idLastMessageOnPage=' + idLastMessage;
