...
$(body).on('click', '#save_avatar_image', function () {
    ...
    var url = '/expert/profile/load-avatar-image?id=' + user_id;
...
$(body).on('hide.bs.modal', '.profile-modal-photo', function () {
    ...
    if (delete_unused_image) {
        ...
        $.ajax({
            url: '/expert/profile/delete-unused-image?id=' + user_id,
            method: 'POST',
...
$(body).on('click', '.update_image', function (e) {
    ...
    var url = '/expert/profile/get-data-avatar?id=' + user_id;
...
setInterval(function(){
    ...
    if ($(body).find('.user_is_online').length > 0) {
        ...
        var url = '/expert/profile/get-user-is-online?id='+user_id;
