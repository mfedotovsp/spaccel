    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if ($(modal).find('.modal-header').find('.modal-header-text-append').length === 0) {
                $(modal).find('.modal-header').append('<div class="modal-header-text-append">Разработка гипотез ценностных предложений</div>');
            }
...
$(body).on('change', 'form#hypothesisCreateForm', function(){
    ...
    if (module === 'contractor') {
        url = '/gcps/save-cache-creation-form?id=0&taskId=' + id_page;
    } else {
        url = '/gcps/save-cache-creation-form?id=' + id_page;
    }
    data = $(this).serialize();
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.count === '1') {
                ...
                if (module === 'contractor') {
                    location.href = '/contractor/gcps/task?id=' + id;
                } else {
