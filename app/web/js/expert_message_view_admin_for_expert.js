...
window.addEventListener('DOMContentLoaded', function() {
    ...
    var linkAllConversation = $('.containerForAllAdminsConversations').find('#adminConversation-'+id_page);
    ...
    if (scrollHeight <= heightScreen - 290) {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-expert.unreadmessage').each(function (index, item) {
                ...
                var url = '/expert/message/read-message-expert?id=' + message_id;
...
setInterval(function(){
    ...
    $.ajax({
        url: '/expert/message/get-list-update-conversations?id=' + conversation_id + '&pathname=view',
        method: 'POST',
        cache: false,
        success: function(response){
    ...
    // Проверяем прочитал ли трекер сообщения
    $(chat).find('.addressee-admin.unreadmessage').each(function (index, item) {

        var message_id = $(item).attr('id').split('-')[1];
        var url = '/expert/message/checking-unread-message-expert?id=' + message_id;
