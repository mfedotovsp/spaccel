...
$(body).on('beforeSubmit', '#create-message-manager', function (e) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.sender === 'manager') {
                if (module === 'admin') $(conversation_list_menu).find(response.blockConversationAdminMain).html(response.conversationAdminMainForAdminAjax);
                else $(conversation_list_menu).find('.containerForAllClientConversations').html(response.conversationsClientAdminForManagerAjax);
            }
            else if (response.sender === 'main_admin') $(conversation_list_menu).find('.containerForManagerConversations').html(response.conversationsManagerForAdminMainAjax);
            ...
            $('#create-message-manager')[0].reset();
...
$(body).on('input', 'textarea#input_send_message', function () {
    ...
    var url = '/' + module + '/message/save-cache-message-manager-form?id=' + conversation_id;
...
$(body).on('click', '.pagination-messages .messages-pagination-list li.next a', function(e){
    ...
    var url = '/' + module + '/message/get-page-message-manager-main-admin?id=' + conversation_id + '&page=' + next_page + '&final=' + idFirstMessagePreviosPage;
...
setInterval(function(){
    ...
    var url = '/' + module + '/message/check-new-messages-manager?id=' + conversation_id + '&idLastMessageOnPage=' + idLastMessage;
