...
function getQueryResponds() {
    ...
    if ($(window).width() <= '480') {
        if (action === 'view-trash') {
            ...
            url = '/responds/get-query-responds?stage=6&id=' + taskId + '&isMobile=true&isModuleContractor=true';
        }
    } else {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=6&id=' + taskId + '&isOnlyNotDelete=false&isModuleContractor=true';
        } else {
            url = '/responds/get-query-responds?stage=6&id=' + taskId + '&isModuleContractor=true';
        }
    }
    $.ajax({
        url: url,
...
$(body).on('input', 'input#search_input_responds_mobile', function () {
    ...
    var url = '/responds/get-query-responds?stage=6&id=' + taskId + '&search=' + search + '&isMobile=true&isModuleContractor=true';
