        ...
        success: function(response){
            ...
            $.each(response.projects, function(){
                ...
                $.ajax({
                    ...
                    url: '/profile/get-result-project?id='+project_id,
                    method: 'POST',
                    cache: false,
                    success: function(response){
...
$(body).on('click', '.container-one_hypothesis', function () {
    ...
    if ($(block_data_project).is(':hidden')){
        ...
        $(this).find('.informationAboutAction').html('Закрыть сводную таблицу проекта');
    }
    if ($(block_data_project).is(':visible')) {
        ...
        $(this).find('.informationAboutAction').html('Посмотреть сводную таблицу проекта');
    }
