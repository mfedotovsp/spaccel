...
$(body).on('beforeSubmit', '#create-message-development', function (e) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.sender === 'user') $(conversation_list_menu).find(response.blockConversationDevelopment).html(response.conversationDevelopmentForUserAjax);
...
$(body).on('input', 'textarea#input_send_message', function () {
    ...
    var url = '/message/save-cache-message-development-form?id=' + conversation_id;
...
$(body).on('click', '.pagination-messages .messages-pagination-list li.next a', function(e){
    ...
    var url = '/message/get-page-message-development?id=' + conversation_id + '&page=' + next_page + '&final=' + idFirstMessagePreviosPage;
