...
// Получаем id беседы техподдержки с пользователем
    ...
    if (scrollHeight <= heightScreen - 290) {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-development.unreadmessage').each(function (index, item) {
                ...
                var url = '/message/read-message-development?id=' + message_id;
...
setInterval(function(){
    ...
    $.ajax({
        url: '/message/get-list-update-conversations?id=' + conversation_id + '&pathname=technical-support',
        method: 'POST',
        cache: false,
        success: function(response){
    ...
    $(chat).find('.addressee-user.unreadmessage').each(function (index, item) {
        ...
        var message_id = $(item).attr('id').split('-')[1];
        var url = '/message/checking-unread-message-development?id=' + message_id;
