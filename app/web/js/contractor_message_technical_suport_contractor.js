...
const simpleBarDataChatContractor = new SimpleBar(document.getElementById('data-chat'));
// Получаем id беседы
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    var unreadmessage = $(body).find('.addressee-contractor.unreadmessage:first');
    if ($(unreadmessage).length) // Первое непрочитанное сообщение для пользователя
        simpleBarDataChatContractor.getScrollElement().scrollTop = $(unreadmessage).offset().top - $(unreadmessage).height() - $('.data-chat').height();
    else
...
$(document).ready(function () {
    ...
    var scrollHeight = simpleBarDataChatContractor.getScrollElement().scrollHeight; // Высота скролла
    if (scrollHeight <= heightScreen - 290) {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-contractor.unreadmessage').each(function (index, item) {
                ...
                var url = '/contractor/message/read-message-development?id=' + message_id;
    ...
    simpleBarDataChatContractor.getScrollElement().addEventListener('scroll', function () {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-contractor.unreadmessage').each(function (index, item) {
                ...
                var scrollTop = simpleBarDataChatContractor.getScrollElement().scrollTop,
                    scrollHeight = simpleBarDataChatContractor.getScrollElement().scrollHeight,
...
setInterval(function(){
    ...
    // Обновляем беседы исполнителя
    $.ajax({
        url: '/contractor/message/get-list-update-conversations?id=' + conversation_id + '&pathname=technical-support',
        method: 'POST',
        cache: false,
        success: function(response){
            ...
            $(conversation_list_menu).find(response.blockConversationDevelopment).html(response.conversationDevelopmentForContactorAjax);
            $(conversation_list_menu).find('.containerForAllUsersConversations').html(response.conversationsUserForContractorAjax);
    ...
    $(chat).find('.addressee-development.unreadmessage').each(function (index, item) {
        ...
        var message_id = $(item).attr('id').split('-')[1];
        var url = '/contractor/message/checking-unread-message-development?id=' + message_id;
