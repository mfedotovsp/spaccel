...
$(window).resize(function () {
    if ($(window).width() <= '480' && $('.confirm-mvp-view').length > 0) {
        location.reload();
        $('.confirm-mvp-view').remove();
...
function getQueryResponds() {
    ...
    if ($(window).width() <= '480') {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=8&id=' + confirmId + '&isMobile=true&isOnlyNotDelete=false&isModuleContractor=' + isContractorModule;
        } else {
            url = '/responds/get-query-responds?stage=8&id=' + confirmId + '&isMobile=true&isModuleContractor=' + isContractorModule;
        }
    } else {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=8&id=' + confirmId + '&isOnlyNotDelete=false&isModuleContractor=' + isContractorModule;
        } else {
            url = '/responds/get-query-responds?stage=8&id=' + confirmId + '&isModuleContractor=' + isContractorModule;
        }
    }
    $.ajax({
        url: url,
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if ($(modal).find('.modal-header').find('.modal-header-text-append').length === 0) {
                $(modal).find('.modal-header').append('<div class="modal-header-text-append">Этап 8. Подтверждение MVP</div>');
            }
...
$(body).on('change', 'form#new_respond_form', function(){
    ...
    var url = '/responds/save-cache-creation-form?stage=8&id=' + confirmId;
    var data = $(this).serialize();
    $.ajax({
        url: url,
        data: data,
...
$(body).on('input', 'input#search_input_responds_mobile', function () {
    ...
    var url = '/responds/get-query-responds?stage=8&id=' + confirmId + '&search=' + search + '&isMobile=true&isModuleContractor=' + isContractorModule;
...
$(body).on('click', '.showRespondUpdateForm', function(e){
    ...
    if (!isContractorModule) {
        if (action !== 'view-trash') {
            url = '/responds/get-data-update-form?stage=8&id=' + id;
        } else {
            url = '/responds/get-data-update-form?stage=8&id=' + id + '&isOnlyNotDelete=false';
        }
    } else {
        $('#formUpdateRespond').remove();
        if (action !== 'view-trash') {
            url = '/contractor/responds/get-data-update-form?stage=8&id=' + id;
        } else {
            url = '/contractor/responds/get-data-update-form?stage=8&id=' + id + '&isOnlyNotDelete=false';
        }
    }
...
$(body).on('click', '.showDescInterviewUpdateForm', function(e){
    ...
    var id = $(this).attr('id').split('-')[1];
    if (action !== 'view-trash') {
        url = '/interviews/get-data-update-form?stage=8&id=' + id;
    } else {
        url = '/interviews/get-data-update-form?stage=8&id=' + id + '&isOnlyNotDelete=false';
    }
...
$(body).on('click', '.showDeleteRespondModal', function(e){
    ...
    var id = $(this).attr('id').split('-')[1];
    var url = '/responds/get-data-model?stage=8&id=' + id;
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $(delete_respond_modal).find('.modal-footer #confirm-delete-respond').attr('href', '/responds/delete?stage=8&id=' + response.id);
...
// Отмена завершения неудачного подтверждения MVP
