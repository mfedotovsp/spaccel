...
$(body).on('change', 'form#new_respond_form', function(){
    ...
    var url = '/responds/save-cache-creation-form?stage=2&id=' + confirmId;
    var data = $(this).serialize();
    $.ajax({
        url: url,
        data: data,
...
$(body).on('click', '.showRespondUpdateForm', function(e){
    ...
    if (!isContractorModule) {
        if (action !== 'view-trash') {
            url = '/responds/get-data-update-form?stage=2&id=' + id;
        } else {
            url = '/responds/get-data-update-form?stage=2&id=' + id + '&isOnlyNotDelete=false';
        }
    } else {
        $('#formUpdateRespond').remove();
        if (action !== 'view-trash') {
            url = '/contractor/responds/get-data-update-form?stage=2&id=' + id;
        } else {
            url = '/contractor/responds/get-data-update-form?stage=2&id=' + id + '&isOnlyNotDelete=false';
        }
    }
...
$(body).on('click', '.showDescInterviewUpdateForm', function(e){
    ...
    var id = $(this).attr('id').split('-')[1];
    if (action !== 'view-trash') {
        url = '/interviews/get-data-update-form?stage=2&id=' + id;
    } else {
        url = '/interviews/get-data-update-form?stage=2&id=' + id + '&isOnlyNotDelete=false';
    }
...
$(body).on('click', '.showDeleteRespondModal', function(e){
    ...
    var id = $(this).attr('id').split('-')[1];
    var url = '/responds/get-data-model?stage=2&id=' + id;
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $(delete_respond_modal).find('.modal-footer #confirm-delete-respond').attr('href', '/responds/delete?stage=2&id=' + response.id);
