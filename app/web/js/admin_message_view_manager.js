...
const simpleBarDataChatManager = new SimpleBar(document.getElementById('data-chat'));
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    var unreadmessage = $(body).find('.addressee-manager.unreadmessage:first');
    if ($(unreadmessage).length) // Первое непрочитанное сообщение для пользователя
        simpleBarDataChatManager.getScrollElement().scrollTop = $(unreadmessage).offset().top - $(unreadmessage).height() - $('.data-chat').height();
    else
        simpleBarDataChatManager.getScrollElement().scrollTop = simpleBarDataChatManager.getScrollElement().scrollHeight;
});
...
$(body).on('click', '.conversation-link', function () {
    ...
    if (($(this).attr('id').split('-')[0] === 'clientAdminConversation')) {
        window.location.href = '/client/message/view?id='+ id + '&type=manager';
    }
});
...
$(document).ready(function () {
    ...
    var scrollHeight = simpleBarDataChatManager.getScrollElement().scrollHeight; // Высота скролла
    if (scrollHeight <= heightScreen - 290) {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-manager.unreadmessage').each(function (index, item) {

                var message_id = $(item).attr('id').split('-')[1];
                var url = '/' + module + '/message/read-message-manager?id=' + message_id;
    ...
    simpleBarDataChatManager.getScrollElement().addEventListener('scroll', function () {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-manager.unreadmessage').each(function (index, item) {
                ...
                var scrollTop = simpleBarDataChatManager.getScrollElement().scrollTop,
                    scrollHeight = simpleBarDataChatManager.getScrollElement().scrollHeight,
...
setInterval(function(){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $(conversation_list_menu).find(response.blockConversationAdminMain).html(response.conversationAdminMainForManagerAjax);
            $(conversation_list_menu).find(response.blockConversationDevelopment).html(response.conversationDevelopmentForManagerAjax);
            $(conversation_list_menu).find('.containerForAllClientConversations').html(response.conversationsClientForManagerAjax);
    ...
    $(chat).find('.addressee-main_admin.unreadmessage').each(function (index, item) {
        ...
        var message_id = $(item).attr('id').split('-')[1];
        var url = '/' + module + '/message/checking-unread-message-manager?id=' + message_id;
