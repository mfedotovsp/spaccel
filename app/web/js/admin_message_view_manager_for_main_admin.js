...
// Получаем id беседы главного админа с админом
var conversation_id = window.location.search.split('=')[1];

var module = (window.location.pathname).split('/')[1];

var body = $('body');
var id_page = window.location.search.split('=')[1].split('&')[0];


// Прокрутка во время работы прелоадера
window.addEventListener('DOMContentLoaded', function() {
    // Прокрутка до блока активной беседы
    var linkAllConversation = $('.containerForManagerConversations').find('#managerConversation-'+id_page);
...
setInterval(function(){
    ...
    $.ajax({
        url: '/' + module + '/message/get-list-update-conversations?id=' + conversation_id + '&pathname=view-manager',
        method: 'POST',
        cache: false,
        success: function(response){
