...
const simpleBarDataChatExpert = new SimpleBar(document.getElementById('data-chat'));
// Получаем id беседы главного админа с экспертом
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    var linkAllConversation = $('.containerAdminMainConversation').find('#conversationTechnicalSupport-'+id_page);
    simpleBarConversations.getScrollElement().scrollTop = $(linkAllConversation).offset().top - 211;
    // Прокрутка блока сообщений
    var unreadmessage = $(body).find('.addressee-expert.unreadmessage:first');
    if ($(unreadmessage).length) // Первое непрочитанное сообщение для пользователя
        simpleBarDataChatExpert.getScrollElement().scrollTop = $(unreadmessage).offset().top - $(unreadmessage).height() - $('.data-chat').height();
    else
        simpleBarDataChatExpert.getScrollElement().scrollTop = simpleBarDataChatExpert.getScrollElement().scrollHeight;
});
...
$(body).on('click', '.container-user_messages', function () {
    ...
    else if ($(this).attr('id').split('-')[0] === 'adminMainConversation') {
        window.location.href = '/expert/message/view?id='+id;
    }
    else if ($(this).attr('id').split('-')[0] === 'adminConversation') {
        window.location.href = '/expert/message/view?id='+id;
...
$(document).ready(function () {
    ...
    var scrollHeight = simpleBarDataChatExpert.getScrollElement().scrollHeight; // Высота скролла
    if (scrollHeight <= heightScreen - 290) {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-expert.unreadmessage').each(function (index, item) {

                var message_id = $(item).attr('id').split('-')[1];
                var url = '/expert/message/read-message-development?id=' + message_id;
    ...
    simpleBarDataChatExpert.getScrollElement().addEventListener('scroll', function () {
        ...
        timeoutReadMessage = setTimeout(function() { //чтобы не искать одно и то же несколько раз
            ...
            $(chat).find('.addressee-expert.unreadmessage').each(function (index, item) {
                ...
                var scrollTop = simpleBarDataChatExpert.getScrollElement().scrollTop,
                    scrollHeight = simpleBarDataChatExpert.getScrollElement().scrollHeight,
...
setInterval(function(){
    ...
    // Обновляем беседы эксперта
    $.ajax({
        url: '/expert/message/get-list-update-conversations?id=' + conversation_id + '&pathname=technical-support',
        method: 'POST',
        cache: false,
        success: function(response){
            ...
            $(conversation_list_menu).find(response.blockConversationAdminMain).html(response.conversationAdminMainForExpertAjax);
            $(conversation_list_menu).find(response.blockConversationDevelopment).html(response.conversationDevelopmentForExpertAjax);
            $(conversation_list_menu).find('.containerForAllAdminsConversations').html(response.conversationsAdminForExpertAjax);
            $(conversation_list_menu).find('.containerForAllUsersConversations').html(response.conversationsUserForExpertAjax);
    ...
    // Проверяем прочитала ли тех.поддержка сообщения
    $(chat).find('.addressee-development.unreadmessage').each(function (index, item) {

        var message_id = $(item).attr('id').split('-')[1];
        var url = '/expert/message/checking-unread-message-development?id=' + message_id;
