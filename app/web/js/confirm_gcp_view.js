...
$(window).resize(function () {
    if ($(window).width() <= '480' && $('.confirm-gcp-view').length > 0) {
        location.reload();
        $('.confirm-gcp-view').remove();
...
function getQueryResponds() {
    ...
    if ($(window).width() <= '480') {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=6&id=' + confirmId + '&isMobile=true&isOnlyNotDelete=false&isModuleContractor=' + isContractorModule;
        } else {
            url = '/responds/get-query-responds?stage=6&id=' + confirmId + '&isMobile=true&isModuleContractor=' + isContractorModule;
        }
    } else {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=6&id=' + confirmId + '&isOnlyNotDelete=false&isModuleContractor=' + isContractorModule;
        } else {
            url = '/responds/get-query-responds?stage=6&id=' + confirmId + '&isModuleContractor=' + isContractorModule;
        }
    }
    $.ajax({
        url: url,
...
$(body).on('change', 'form#new_respond_form', function(){
    ...
    var url = '/responds/save-cache-creation-form?stage=6&id=' + confirmId;
    var data = $(this).serialize();
    $.ajax({
        url: url,
        data: data,
...
$(body).on('input', 'input#search_input_responds_mobile', function () {
    ...
    var url = '/responds/get-query-responds?stage=6&id=' + confirmId + '&search=' + search + '&isMobile=true&isModuleContractor=' + isContractorModule;
...
$(body).on('click', '.showRespondUpdateForm', function(e){
    ...
    if (!isContractorModule) {
        if (action !== 'view-trash') {
            url = '/responds/get-data-update-form?stage=6&id=' + id;
        } else {
            url = '/responds/get-data-update-form?stage=6&id=' + id + '&isOnlyNotDelete=false';
        }
    } else {
        $('#formUpdateRespond').remove();
        if (action !== 'view-trash') {
            url = '/contractor/responds/get-data-update-form?stage=6&id=' + id;
        } else {
            url = '/contractor/responds/get-data-update-form?stage=6&id=' + id + '&isOnlyNotDelete=false';
        }
    }
...
$(body).on('click', '.showDescInterviewUpdateForm', function(e){
    ...
    var id = $(this).attr('id').split('-')[1];
    if (action !== 'view-trash') {
        url = '/interviews/get-data-update-form?stage=6&id=' + id;
    } else {
        url = '/interviews/get-data-update-form?stage=6&id=' + id + '&isOnlyNotDelete=false';
    }
