...
$(body).on('beforeSubmit', '#create-message-admin', function (e) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.sender === 'user') $(conversation_list_menu).find(response.blockConversationAdmin).html(response.conversationAdminForUserAjax);
            else if (response.sender === 'admin') $(conversation_list_menu).find('.containerForAllConversations').html(response.conversationsUserForAdminAjax);
...
$(body).on('input', 'textarea#input_send_message', function () {
    ...
    var url = '/message/save-cache-message-admin-form?id=' + conversation_id;
...
$(body).on('click', '.pagination-messages .messages-pagination-list li.next a', function(e){
    ...
    var url = '/message/get-page-message?id=' + conversation_id + '&page=' + next_page + '&final=' + idFirstMessagePreviosPage;
...
setInterval(function(){
    ...
    var url = '/message/check-new-messages-admin?id=' + conversation_id + '&idLastMessageOnPage=' + idLastMessage;
