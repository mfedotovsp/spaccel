...
<?php if (count($models) > 0): ?>
    ...
    <?php foreach ($models as $model) : ?>
                    ...
                    <div class="row pull-right display-flex align-items-center pr-10">
                        <div>
                            <?php if (User::isUserContractor(Yii::$app->user->identity['username']) && in_array($task->getStatus(), [ContractorTasks::TASK_STATUS_NEW, ContractorTasks::TASK_STATUS_PROCESS, ContractorTasks::TASK_STATUS_RETURNED], true)): ?>
                                ...
                                <?= Html::a(Html::img('/images/icons/icon_update.png', ['style' => ['width' => '24px', 'margin-right' => '20px']]),['/contractor/problems/get-hypothesis-to-update', 'id' => $model->getId()], [
                                    'class' => 'update-hypothesis',
                                    'title' => 'Редактировать',
                                ]) ?>

                                <?= Html::a(Html::img('/images/icons/icon_delete.png', ['style' => ['width' => '24px']]),['/contractor/problems/delete', 'id' => $model->getId()], [
        ...
        <div class="hypothesis_table_mobile mt-15">
            <div class="row container-one_hypothesis_mobile row_hypothesis-<?= $model->getId() ?>">
                ...
                <div class="hypothesis_buttons_mobile">
                    ...
                    <?php if (User::isUserContractor(Yii::$app->user->identity['username']) && in_array($task->getStatus(), [ContractorTasks::TASK_STATUS_NEW, ContractorTasks::TASK_STATUS_PROCESS, ContractorTasks::TASK_STATUS_RETURNED], true)): ?>
                        ...
                        <?= Html::a('Редактировать', ['/contractor/problems/get-hypothesis-to-update', 'id' => $model->getId()], [
                        ...
                        <?= Html::a('Удалить проблему', ['/contractor/problems/delete', 'id' => $model->getId()], [
...
<?php else: ?>
    ...
    <h4 class="text-center">Проблемы отсутствуют ...</h4>
